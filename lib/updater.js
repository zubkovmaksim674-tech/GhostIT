...
// Args are passed on the command line (not embedded in the script text) so that
// Cyrillic/spaced temp paths survive the OEM codepage of cmd.exe batch parsing.
const SETUP_SCRIPT = [
  '@echo off',
  'set /a TRIES=0',
  ':wait',
  ...
  'set /a TRIES+=1',
  'if %TRIES% geq 90 goto install',
  'ping -n 2 127.0.0.1 >nul',
  'goto wait',
  ':install',
  'start "" /wait "%~f2" /S',
  'set RC=%ERRORLEVEL%',
  'if not "%RC%"=="0" echo setup_failed rc=%RC% >> "%~f4"',
  'if not "%RC%"=="0" goto relaunch',
  'del /f /q "%~f2"',
  ':relaunch',
  'start "" "%~f3"',
  'del /f /q "%~f0"'
...
function installSetupLocal(setupFile, appExePath, dir) {
  const script = path.join(dir, 'ghostit-update.cmd');
  const logPath = path.join(dir, 'ghostit-update.log');
  try { fs.rmSync(script, { force: true }); } catch {}
  try { fs.rmSync(logPath, { force: true }); } catch {}
  fs.writeFileSync(script, SETUP_SCRIPT, 'ascii');
  try {
    const child = spawn('cmd.exe', ['/c', script, String(process.pid), setupFile, appExePath, logPath], {
      detached: true,
      windowsHide: true,
      stdio: 'ignore'
    });
...
module.exports = { checkLatest, downloadUpdate, installUpdate, installSetupLocal, currentExePath, RELEASES_PAGE, parseVersion, isNewer, SETUP_SCRIPT };
