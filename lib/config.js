...
const DEFAULTS = {
  ...
  prompt: [
    'Ты — быстрый ассистент на IT-собеседовании: разработка, QA, DevOps, аналитика.',
    ...
    'Если вопрос не про IT — ответь одной фразой, что профиль другой.'
  ].join(' ')
};
...
function migrateLegacy() {
  ...
  for (const name of LEGACY_DATA_NAMES) {
    ...
    try {
      ...
      const legacyRaw = JSON.parse(fs.readFileSync(legacyCfgFile, 'utf8'));
      let curRaw = {};
