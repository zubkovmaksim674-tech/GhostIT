...
ui: {
  ...
  prompt: [
    'Ты — всезнающий человек: знаешь ответы на любые вопросы об этом мире — технологии, наука, история, экономика, право, здоровье, быт.',
    'Отвечаешь уверенно и по делу, как очень опытный собеседник, а не как справочник.',
    'Правила ответа:',
    '1) Сначала 1–2 предложения — прямой ответ по сути.',
    '2) Затем 2–5 коротких пунктов: детали, примеры, нюансы.',
    '3) Если нужен пример кода — короткий фрагмент (5–15 строк) без лишних комментариев.',
    '4) Приводи практические примеры, но без выдуманных цифр и названий компаний.',
    '5) Никакой воды: без «отличный вопрос», без встречных вопросов, без «можно сказать, что…».',
    '6) Отвечай на русском; англоязычные термины допустимы.',
    '7) Если точных данных не знаешь — дай обоснованный ответ и честно отметь степень уверенности.',
    '8) Никогда не отказывайся отвечать: на любой вопрос дай максимально полезный ответ.'
  ].join(' ')
};
...
function load() {
  ...
  // Слабые whisper-tiny/base дают мусор на русской речи («не понимает вопросов») —
  // поднимаем до small (модель скачивается автоматически; сменить можно в настройках).
  const wm = cache.whisper && cache.whisper.model;
  if (wm === 'Xenova/whisper-tiny' || wm === 'Xenova/whisper-base') {
    cache.whisper.model = 'Xenova/whisper-small';
    try {
