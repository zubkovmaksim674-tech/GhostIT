...
async function streamAnswer(options, handlers) {
  ...
  if (!res.ok) {
    ...
    if (parsed && parsed.error && parsed.error.message) {
      ...
      const snippet = String(detail || '').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 120);
      message = 'HTTP ' + res.status + (snippet ? ': ' + snippet : '');
    }
...
module.exports = { streamAnswer, buildMessages, normalizeBaseUrl };
